...
<head>
	<meta charset="UTF-8">
	<title><?php if(isset($site_name)) { echo $site_name; } ?> - Accidental / Emergency Orders - Admin Panel</title>
	...
	<div class="wrapper row-offcanvas row-offcanvas-left">
		...
		<aside class="right-side">
			<section class="content-header">
				<h1>
					Dashboard
					<small>Accidental / Emergency Orders</small>
				</h1>
				<ol class="breadcrumb">
			...
			<section class="content">
				...
				<label class="selected-options">&nbsp;&nbsp;&nbsp;&nbsp;Filter Emergency Orders</label>
				<div class="row">
					<div class="col-xs-12">
						<div class="col-xs-4">
							<div class="checkbox">
					...
					<table id="example1" border="0" cellpadding="0" cellspacing="0" class="table custom-table">
						<thead>
							<tr>
								...
								<th class="last"><i class="fa fa-bookmark"></i> &nbsp;&nbsp;Location Name</th>
								<th><i class="fa fa-hand-o-up"></i> &nbsp;&nbsp;Clear</th>
							</tr>
						</thead>
						<tbody>
							<?php if(isset($rows) && count($rows) > 0) { $count = 0; foreach($rows as $row) { ?>
							<tr id="<?php echo $count; ?>">
								<td style="display:none;" id="clear_row_<?php echo $row['EmgOrderId']; ?>" data-search="<?php echo $row['isCleared']; ?>"><?php echo $row['Email']; ?></td>
								...
								<td>
									<div class="checkbox"><label class="paymode">
										<input type="checkbox" data-checked="<?php echo $row['isCleared']; ?>" id="clear_<?php echo $row['EmgOrderId']; ?>" name="clear" value="<?php echo $row['EmgOrderId']; ?>"<?php if($row['isCleared'] == 1) { echo ' checked'; } ?>></input>
									</label></div>
								</td>
							</tr>
							<?php $count += 1; } } ?>
...
<script type="text/javascript">
		...
		if(checked == '1') {
			swal({
				...
				if (isConfirm) {
					$.ajax({
						type: "POST",
						url: "/admin/orders/clear_emg_order",
						data: { "EmgOrderId": id, "isCleared": 0 }, 
						dataType: "text",
			...
			swal({
				...
				if (isConfirm) {
					$.ajax({
						type: "POST",
						url: "/admin/orders/clear_emg_order",
						data: { "EmgOrderId": id, "isCleared": 1 }, 
						dataType: "text",
						success: function(data) {
							$('#clear_' + id).attr("data-checked", '1');
	...
	$('#search').on('click', function(e) {
		var form = '<form action="/admin/orders/emgorders" method="POST">';
